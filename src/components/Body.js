...
if(listOfRestaurants.length===0)
{
  ...
  const searchCall = () => {
    ...
    //filter the restauraunt cards and update the UI & search Text
    ...
    return (
      ...
      {foodItems.map((banner) => (
        <div key={banner.id} className="banner-item" >
          <img src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_288,h_360/${banner.imageId}`}   alt={banner.description} />
        </div> 
      ))}
    </div>
    <div>
      ...
      {topRatedChainsbanner.map((restaurant)=>(
        ...
        <div className="res-container">
          ...
          <Link 
            ...
            to={"/restauraunts/"+restaurant.info.id}
          >
